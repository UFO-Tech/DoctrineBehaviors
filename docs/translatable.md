...
/** @var \VasyaXY\DoctrineBehaviors\Contract\Entity\TranslatableInterface $category */
...
This library provides an interface `VasyaXY\DoctrineBehaviors\Contract\Provider\LocaleProviderInterface` that [by default returns the current locale using Symfony `RequestStack` or `TranslatorInterface`](https://github.com/VasyaXY/DoctrineBehaviors/blob/master/src/Provider/LocaleProvider.php).
